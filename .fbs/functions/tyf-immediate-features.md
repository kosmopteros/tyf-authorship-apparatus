...
- `tyf-helper-current-contract`: the TYF helper smoke suite exercises workspace safety, a plain-language first-session start path, provenance-safe capture, visible reflexes, explicit git snapshots, controlled writes, documentation honesty, and update checks without writing manuscript text outside `tyf write --confirm` {formulated_by=req:f84779aa;req:b2b3df90, axes=bad-outcome;edge;boundary;integration, na=security:local-author workspace helper with no network service or auth boundary;non-functional:no throughput or latency budget for first-day writing setup}
