...
This check is your responsibility, not the author's. Do not ask the author to invoke skills or remember helper commands. Treat authorship, source, draft, feedback, character, continuation, and manuscript requests as TYF requests automatically. On session entry or return-after-time, run `tyf resume` as hidden amanuensis machinery before choosing the next move. If the host has wired the session-start hook, `tyf hook session-start` supplies the same read-only context automatically.
