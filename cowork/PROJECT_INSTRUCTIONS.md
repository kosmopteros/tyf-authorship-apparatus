...
Advanced helper commands include `init`, `status`, `resume`, `new-work`, `start`, `begin`, `import`, `capture`, `structure`, `attend`, `character`, `consult-character`, `open`, `mark-ready`, `propose`, `audit`, `review`, `accept`, `adopt`, `write`, `doctor`, `reflexes`, and `snapshot`. Cowork v1 should stay root-first unless the author deliberately asks to manage multiple works from one folder. Source captures and textual imports mint source fragments, `structure` turns explicit source material into knowledge records and an amanuensis brief, `attend` turns that structured material into source-grounded gentle next questions, and source-grounded proposals should carry fragment ids with `--source-ref`. The helper is still the single apparatus writer into `manuscript/`. `tyf resume` shows continuity, `tyf reflexes` shows visible hooks, and `tyf snapshot --message "..."` makes an explicit git recovery commit when the workspace is a git repo. TYF never commits silently.
