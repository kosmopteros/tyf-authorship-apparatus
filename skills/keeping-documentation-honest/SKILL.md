...
- **Automatically, warn-only, as a tail step of every mutating `tyf` command** (`init`, `new-work`, `start`, `begin`, `import`, `capture`, `propose`, `audit --record`, `accept`, `adopt`, `write`, `mark-ready`). The trigger is wired into the command itself, not into git or author memory, because a workspace may not be a repo and committing is a manual act that can be forgotten. The hook never blocks the action; it surfaces drift at the moment structure changes. Silence it with `TYF_NO_DOC_HOOK=1`.
