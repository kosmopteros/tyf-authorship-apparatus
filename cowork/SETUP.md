...
3. In a task, paste: "Use TYF to start my new book. Set up the workspace if needed, do not draft manuscript prose yet, do not block on a title, and ask me the first source questions. If I provide existing material, preserve it first and show me the orientation packet."

Claude should load `using-tyf`, run `tyf init` if the folder is not a workspace yet, then run `tyf start` if the title is unknown or `tyf start "Working Title"` if the author already has one. If a chat export, folder, old workspace, or zip arrives, it should run `tyf import <path>` and read the orientation packet before organizing anything. It should tell you what files were created in plain language.
