...
`tyf workbench` writes a local Draft Review Workbench under `.review/workbench/`. The static workbench shows `drafts/candidate-draft.md`, approved `manuscript/` units, `design/book-style.yaml`, the running style sheet, and `assets/images/`. `tyf workbench --serve` may save the candidate draft from the browser, but only when the draft's loaded base hash still matches the file on disk; stale saves are conflicts, not overwrites. It may create review packets for the Gate. It never writes to `manuscript/`.
...
- Treating `tyf workbench --serve` as a manuscript editor instead of a draft workbench.
...
tyf workbench [work]     # local draft/manuscript workbench; --serve saves drafts only
