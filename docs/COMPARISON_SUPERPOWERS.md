...
| `requesting-code-review` / `receiving-code-review` | `editing-faithfully` (partial) | Review feedback ignored, or applied without judgement. Superpowers has a dedicated two-sided review protocol. TYF has the *faithful editor* but no explicit "how to receive and triage critique" discipline. Gap. |
...
1. **A real state and memory layer.** Superpowers' state is largely the git repo and the plan files. TYF has an explicit workspace contract, an idempotent initializer, a SQLite apparatus-memory with an event log, and the content-addressed notice ledger. For a domain without git, this is more than superpowers offers, and the attentive-amanuensis loop (surface-only, never modifies, dismissed-with-resurface) has no superpowers equivalent.
2. **The write boundary as a hard contract.** Superpowers trusts the agent to follow TDD. TYF makes the manuscript physically reachable only through `tyf write --confirm`, with `tyf doctor` detecting out-of-band edits. For irreplaceable authored prose this is a stronger guarantee than "please follow the methodology."
