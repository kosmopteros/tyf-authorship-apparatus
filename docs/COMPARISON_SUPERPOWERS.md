...
| `systematic-debugging` | `diagnosing-text` + `tyf diagnose` (partial) | Guessing at causes instead of isolating them. TYF now has a systematic isolation packet for "why does this section fail to land": diagnostic band, reader symptom, source/register reminders, cause hypotheses, and one next experiment. It is still newer and softer than superpowers' debugging loop because prose diagnosis is judgment-based. |
...
1. **Partial proof is partial proof.** The command-backed helper contract is no longer paper-only: 128/128 development scenarios have direct RED proof, and the stdlib helper suite has 164 tests including source-grounded attention packets with transparent local retrieval, external-feedback triage, continuing-work packets, doctor repair-boundary coverage, diagnostic-isolation packets, private-context-free author/root/runtime surfaces, exported release-tree check/install smoke coverage, and a fresh exported Codex install opening a separate book workspace from an arrival scaffold. Superpowers earned its confidence through usage; TYF still has only a first prompt-level subagent pressure run. GREEN passed 11/11, but the RED baseline was weak, so skill-pressure proof remains partial rather than settled.
