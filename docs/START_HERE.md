...
7. Use the generated gentle attention deck without making the author answer every prompt; when focus matters, use the packet's transparent local retrieval section; ask one passage-opening question at a time.
8. Draft candidates in `drafts/`; keep `manuscript/` empty until explicit controlled write.
