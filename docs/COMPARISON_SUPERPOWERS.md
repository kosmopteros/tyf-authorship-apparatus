...
| `executing-plans` / `subagent-driven-development` | `continuing-the-work` (partial) | Execution drifts from the plan. TYF now has a session-continuity loop that writes one next move and a stop condition, but it is still much lighter than superpowers' plan-execution discipline. |
...
| `systematic-debugging` | `diagnosing-text` + `tyf diagnose` (partial) | Guessing at causes instead of isolating them. TYF now has a systematic isolation packet for "why does this section fail to land": diagnostic band, reader symptom, source/register reminders, mechanical diagnostic frame prompts, and one next experiment. It is still newer and softer than superpowers' debugging loop because prose diagnosis is judgment-based. |
...
2. **Execution and parallelism.** Superpowers sustains long multi-session builds (plan execution, subagent-driven development, parallel dispatch). TYF now has a first continuity loop for a sitting and a diagnostic isolation pass, but not the mature long-arc execution machinery superpowers has for software.
...
1. **Partial proof is partial proof.** The command-backed helper contract is no longer paper-only: 175/175 development scenarios have direct RED proof, and the core helper suite has 197 tests, with 230 aggregate pytest cases across the helper and Workbench modules, including a reproducible first-sitting rehearsal, existing-work recovery packets for formatted and illustrated arrivals, language-neutral structure records for non-English source, answered-prompt resume handling, source-grounded attention packets with transparent local retrieval, external-feedback triage, continuing-work packets, typographer-redactor treatment packets, live Draft Review Workbench generation with locked draft-save conflict protection, Workbench bridge containment, rebuildable book-graph projections, concept/continuity/polish review wrapper coverage, automatic session-start and prompt-submit context, Codex and Claude hook manifests with TYF-identifying status messages, doctor repair-boundary coverage, diagnostic-isolation packets, private-context-free author/root/runtime surfaces, exported release-tree check/install smoke coverage, release manifest context-path validation, hidden portability-doc drift checks, pressure-eval honesty checks, and a fresh exported Codex install opening a separate book workspace from an arrival scaffold. Superpowers earned its confidence through usage; TYF still has only a first prompt-level subagent pressure run. `scripts/tyf_pressure_eval.py` now grades that evidence as partial: GREEN passed 11/11, but RED coverage is 5/11 cases, RED failures are 1/5 sampled baseline trials, and transcripts are choice-table excerpts.
2. **Surface-area-to-usage ratio.** TYF has 19 skills, a broad helper CLI, a SQLite layer, visible reflex hooks, and an opt-in LLM pass, for zero real users. Superpowers grew its surface from usage. TYF risks being over-engineered for problems no author has actually hit.
...
1. **Repeat and strengthen the prompt-level RED/GREEN loop.** This is non-negotiable. The command-backed Be loop now has direct RED proof, and `scripts/tyf_pressure_eval.py --require-strong` prevents overclaiming, but the first subagent pressure run is still thin. TYF needs stronger RED baselines, repeated harness runs, and full transcripts showing failures when the skills are absent or constrained.
2. **Real long-arc continuing-work usage.** The session packet exists now; the next proof is watching it sustain a real book across weeks without becoming a productivity system.
...
Superpowers is a proven methodology; TYF is a promising architecture with stronger local evidence than before. TYF's state, memory, feedback-triage, continuity packet, resume return-context recovery, diagnostic isolation, and write-boundary design are genuinely ahead of superpowers for a no-git authored-prose domain, and its documentation-honesty mechanization is a real contribution. But superpowers has the only currency that ultimately counts, real usage, and TYF still lacks mature parallelism, update distribution, and broad pressure history. The most honest one-line summary: TYF has out-designed superpowers in its niche and now proves its helper contract, but it is still under-used and under-pressure-tested relative to superpowers in practice. The next move is not more architecture. It is repeated author and harness proof.
