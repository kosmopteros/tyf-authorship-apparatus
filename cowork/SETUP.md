...
2. Paste the contents of `cowork/PROJECT_INSTRUCTIONS.md` into the project's instructions. This loads the commitments, automatic skill routing, and the write zones at the start of every task.
3. In a task, paste: "Use TYF to help me start writing my new book today. Set up the workspace if needed, treat TYF as automatic hidden machinery, do not block on a title, preserve any existing material first, show me the orientation and writing runway, structure any minted text source fragment before drafting, and help me begin one candidate passage in drafts with gentle questions and without writing manuscript text."
