...
- `tyf-helper-current-contract`: the TYF helper smoke suite exercises workspace safety, a plain-language first-session start path, provenance-safe capture, visible reflexes, explicit git snapshots, Gate-controlled writes, documentation honesty, and update checks without writing manuscript text outside proposal, audit, author decision with acceptance evidence, and `tyf write --decision` records {formulated_by=req:d6e294d5;req:b2b3df90, axes=bad-outcome;edge;boundary;integration;security, na=non-functional:no throughput or latency budget for first-day writing setup}
