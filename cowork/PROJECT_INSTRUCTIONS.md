...
Use `tyf` for file operations. For a new book or body of work where the author wants to write today, prefer `tyf today`; if the author brings an existing chat export, folder, old workspace, zip, or scaffold, run `tyf today <path>`. Then read the orientation and `.review/today.md`, ask only what is needed to begin one candidate passage, and draft in `drafts/today-draft.md`. Do not hand the author an advanced command list unless they ask for one.
