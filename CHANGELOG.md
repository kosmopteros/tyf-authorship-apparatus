...
- Added the first Draft Review Workbench slice: `tyf surface` generates a local draft/manuscript review table with book-style and image-asset scaffolding, and `tyf surface --serve` can save candidate draft edits with base-hash conflict protection while keeping `manuscript/` read-only.
