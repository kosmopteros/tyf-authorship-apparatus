...
**A. As a plugin (recommended for sharing).** Package this repository as a Claude plugin and install it from the plugin marketplace, the same way other Cowork plugins are installed. The plugin bundles all seventeen skills.
...
In a new task, ask Claude to list its TYF skills and to state the write zones. It should return all seventeen skills, route any authorship request through `using-tyf`, and refuse to write into `manuscript/` outside `tyf write --decision`.
