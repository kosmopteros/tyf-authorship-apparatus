---
name: interviewing-the-author
description: Use when the author needs to think aloud, be interviewed, or surface tacit knowledge; when a thesis is vague or missing; when an argument exists in the author's head but not on paper; or when voice and register preferences need eliciting
---
...
**Eliciting structure.** Ask where the idea is in its life: a sentence, an outline, a half-manuscript. Ask what order the author imagines, if any. Capture it as a candidate, not a decree.

## Rationalization table

| What you will tell yourself | The reality | Do instead |
|---|---|---|
| "I can write a strong thesis for them to react to." | A thesis you wrote is your argument wearing their name. | Ask the questions that force them to state theirs in one paragraph. |
...
- Writing the author's thesis and presenting it as theirs.
- Moving to drafting while the central tension is still fuzzy.
- Choosing an outline the author never confirmed.
- Treating one cooperative "sure" as a sharp argument.

## Output shape

```markdown
## Interview record
Thesis (author's words, one paragraph):
Tension it resolves:
Personal stake:
Core concepts surfaced:
Examples surfaced (not previously written):
Register preferences / exemplars:
Structure the author imagines (candidate):
Open questions raised:
```

## Next

Feed concepts and examples to `structuring-knowledge`. Feed voice material to `managing-voice`. Only when thesis, structure, and register exist does `composing-as-amanuensis` become available.
