...
**v0.6.4 "Workbench Style Surface".**
...
For Codex specifically, TYF has two layers. Install the TYF skills once into `$CODEX_HOME/skills` or `~/.codex/skills` so `$using-tyf` is available. If you use TYF as a Codex plugin, also refresh the personal plugin cache after updating the pack:

```
bash scripts/install.sh codex-plugin
```
...
That writes the current release to `$CODEX_HOME/plugins/cache/personal/tyf/<version>` or `~/.codex/plugins/cache/personal/tyf/<version>` and removes older TYF personal plugin cache versions; restart Codex so it reloads the plugin skills and hook manifest. The Codex plugin ships the skills and hook manifest; its automatic hooks identify themselves with `TYF:` status messages, but they emit context only inside an initialized TYF workspace. The generated `AGENTS.md` carries the same routing for book repos. Then, inside each book repo, `tyf init` writes the local `AGENTS.md` workspace contract so Codex entering that repo knows to run `tyf resume`, `tyf start`, or `tyf start <path>` before drafting. If the author uses the local Workbench with Codex MCP, run `tyf workbench --codex-mcp-config` once inside the book folder to write the ready `$CODEX_HOME/config.toml` or `~/.codex/config.toml`; no hand-editing of absolute paths is required. The MCP server is workspace-bound and exposes no tools when Codex is launched outside that book folder. The author should not need to invoke a skill by name after the workspace context exists.
...
tyf workbench [work] [--serve] [--port <n>] [--open] [--refresh-map] [--codex-mcp-config]
...
tyf hook session-start   # emit read-only host context inside a TYF workspace
tyf hook message-sent    # emit read-only prompt context inside a TYF workspace
...
The Draft Review Workbench is the first author-facing book surface. `tyf workbench` now generates the live local desk at `.review/workbench/workbench-live.html` and `.review/workbench/workbench-live-data.json`, showing multiple `drafts/*.md` units beside read-only `manuscript/*.md` units with `design/book-style.yaml`, the running style sheet, Codex/bridge status, save-safety state, approval state, review summaries, and `assets/images/` visible in a side panel. `tyf workbench --serve` starts a loopback browser workbench that can save draft units directly, but saves are locked and compare-and-swap guarded: the loaded draft hash must still match the file on disk, or TYF reports a conflict instead of overwriting another edit. The browser also warns before discarding unsaved draft changes. The workbench can create new draft units, preserve author notes in `knowledge-base/author-notes.jsonl`, turn notes into footnote candidates, prepare `.review/gate-packets/` only from saved draft text, and write active-context packets for Codex or another amanuensis. `tyf workbench --codex-mcp-config` writes a ready Codex user config block to `$CODEX_HOME/config.toml` or `~/.codex/config.toml`, using the current pack path and book workspace path, so Codex can see active selection/context without the author editing a TOML sample. The server exposes no tools when Codex is launched outside that book workspace. It never writes to `manuscript/`; accepted text still moves through proposal, audit, author review, author decision, and `tyf write --decision`. The scaffolding is intentionally modest: typeface, paragraph style, image-use metadata, visual review, and live assistant status now; true print-ready PDF, KDP trim/bleed preflight, and publisher templates later.
...
`tyf reflexes` shows the apparatus behavior that would otherwise be easy to forget: the read-only session-start context hook, the read-only message-sent prompt hook, the documentation-honesty tail hook, the attentive-amanuensis notice hook after controlled writes, the doctor integrity check, and the git recovery path. `tyf hook session-start` emits host-injectable JSON context with the active work status and automatic author reflexes only when the current folder is an initialized TYF workspace. `tyf hook message-sent` reads a submitted prompt from host JSON and adds routing context only inside an initialized TYF workspace, and only for continuation, arrivals, character questions, and Gate-adjacent prompts. Outside a TYF workspace, both hooks emit nothing; initialization must be an explicit `tyf init` action, not a global message reflex. Neither hook mutates the workspace. If a workspace is also a git repository, mutating commands surface changed-path counts and point to `tyf snapshot`. `tyf snapshot --message "..."` stages and commits the current workspace as an explicit recovery point. TYF never commits silently.
...
- `docs/RELEASE_ACCEPTANCE_REVIEW.md`: evidence-led product disposition note for the current beta release candidate
