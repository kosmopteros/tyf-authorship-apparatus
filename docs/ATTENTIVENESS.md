...
3. **Per-message (opt-in, harness-specific).** The ideal is to route intent continuously as the author and the assistant exchange messages, so continuation, arrival, character, and Gate-adjacent requests trigger the right hidden machinery without the author naming a skill. Where a host exposes a prompt-submit hook, `tyf hook message-sent` reads the submitted prompt from stdin JSON and adds read-only routing context when useful, but only inside an initialized TYF workspace. Outside a TYF workspace it emits nothing, even for book-looking prompts, because `tyf init` is the explicit opt-in boundary. This is context injection, not logging, and it does not mutate the workspace. A separate post-turn intent-capture hook would be a later, explicit choice with a known cost. Until then, per-run and per-write cover the durable notice cases.
