...
**The `tyf` helper** performs the concrete file operations so the agent does not freelance, and it is the single writer into `manuscript/`. Commands include `init` (idempotent: creates only missing structure, never clobbers), `start`, `begin`, `import`, `capture`, `attend`, `session`, `diagnose`, `treat`, `workbench`, `resume`, `status`, `new-work`, `open`, `mark-ready`, `propose`, `audit`, `accept`, `adopt`, `write --decision`, `doctor [--repair]`, `check`, `notice`, `dismiss`, and `reconcile`.

**The Draft Review Workbench** is the first local book surface. `tyf workbench` generates an inspectable HTML/JSON table under `.review/workbench/`: candidate draft on one side, approved manuscript units on the other, and book-style plus image-asset context alongside them. `tyf workbench --serve` may save `drafts/candidate-draft.md`, but only by matching the loaded base hash against the current file; concurrent edits become conflicts. The manuscript pane is read-only. Moving candidate text into the manuscript still requires the Gate.
